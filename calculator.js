...
var calculator = (function () {
    ...
    function filter(event) {
        ...
        if (event === "Ans" && lastNum(outputValue.innerHTML)
            .length === 0) {
            return addValue(answerBtn.value);
        }
        if (event === "." && lastNum(outputValue.innerHTML)
            .indexOf(".") === -1) {
            return addValue(event);
        }
    ...
    function addValue(number) {
        const inputArray = (outputValue.classList.contains("answer") || outputValue.innerHTML === "ERROR") ?
            [number] : [...outputValue.innerHTML, number];
    ...
    const multiply = (number1, number2) => {
        ...
        return parseFloat(product.toFixed(decimalPlaces(number1) +
            decimalPlaces(number2)));
    };
